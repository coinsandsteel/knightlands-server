...
class PlayerController extends IPaymentListener {
    constructor(socket) {
        ...
        this._socket.on(Operations.PublishRaid, this._gameHandler(this._publishRaid.bind(this)));
        ...
        this._socket.on(Operations.BattleSetAdventuresDifficulty, this._gameHandler(this._battleSetAdventuresDifficulty.bind(this)));
    ...
    async _publishRaid(user, data) {
        return this._raidManager.publishRaid(user.id, data.raidId);
    }
    ...
    async _sDungeonWithdrwal(_, data) {
        if (typeof data.to !== 'string') {
            throw Errors.IncorrectArguments;
        }
    ...
    async _sDungeonPurchase(_, data) {
        if (!data.type || !["dungeon", "energy"].includes(data.type)) {
            throw Errors.IncorrectArguments;
        }

        return this.simpleDungeon.purchase(data.type);
    }
